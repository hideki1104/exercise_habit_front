...
export const UserEdit: React.FC<UserEditProps> = () => {
  ...
  useEffect(() => {
    ...
    const connectGetWeightInfo = async () => {
      const responseWeightData = await connectGet(`${constDefine.BASE_URL()}/weights/${userWeightData.id}`);
  ...
  const connectUpdateUserInfo = async (updateData:UserData, weight:number|null) => {
    if (Object.keys(updateData).length) {
      await connectPatch(`${constDefine.BASE_URL()}/users/${userData.data.id}`, updateData);
    }

    if (weight) {
      await connectPatch(`${constDefine.BASE_URL()}/weights/${userWeightData.id}`, {'weight':weight});
    }
  }
