...
export const Top: React.FC<TopProps> = ({isSignUp}) => {
  ...
  const handleUserInfoRegistration = async (userData:UserData, weight:number) => {
    ...
    const responseUserData = await connectPatch(`${constDefine.BASE_URL()}/users/${userInfoData.data.id}`, userData);
    ...
    const reaponseWeightData = await connectPost(`${constDefine.BASE_URL()}/weights`, {'weight': weight, 'user_id': userInfoData.data.id});
  ...
  const handleTrainingTypeRegistration = async (trainingType:number) => {
    ...
    const responseData = await connectPatch(`${constDefine.BASE_URL()}/users/${userInfoData.data.id}`, {'training_type': trainingType});
