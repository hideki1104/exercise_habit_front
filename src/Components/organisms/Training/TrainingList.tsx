...
export const TrainingList: React.FC<TrainingListProps> = ({ isAdmin = false }) => {
  ...
  const connectDeleteTraining = async (index: number) => {
    ...
    const responseTraining = await connectDelete(`${constDefine.BASE_URL()}/trainings/${trainigId}`);
  ...
  useEffect(() => {
    const connectGetTrainingList = async () => {
      const responseTrainingList = await connectGet(`${constDefine.BASE_URL()}/trainings`);
