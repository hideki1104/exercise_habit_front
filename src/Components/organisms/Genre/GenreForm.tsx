...
export const GenreForm: React.FC<GenreFormProps> = () => {
  ...
  const handleOnSubmit: SubmitHandler<GenreData> = async (requestData: GenreData) => {
    const responseData = await connectPost(`${constDefine.BASE_URL()}/genres`, requestData);
    // エラーの場合
  ...
  const connectDeleteGenre = async (index:number) => {
    const responseData = await connectDelete(`${constDefine.BASE_URL()}/genres/${genreList[index].id}`);
    // エラーの場合
    if (!responseData.isSuccess) {
      // エラー処理
