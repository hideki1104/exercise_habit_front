...
export const WeightManagement: React.FC<WeightManagementProps> = () => {
  ...
  useEffect(() => {
    const connectGetWeightInfo = async () => {
      const responseWeightData = await connectGet(`${constDefine.BASE_URL()}/weights`);
  ...
  const connectGetMonthlyWeightData = async (month:string) => {
    const responseWeightData = await connectGet(`${constDefine.BASE_URL()}/weights/${month}/edit`);
